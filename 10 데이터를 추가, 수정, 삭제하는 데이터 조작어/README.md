...
|실수에서는 NULL을 직접 명시해야 하는 경우에 공백 문자열인 작은따옴표를 사용하여 NULL을 입력하는 방식보다 NULL이란 단어를 정확히 입력하는 방식을 대부분 선호합니다. 이 방식을 선호하는 이유는 데이터베이스에 익숙하지 않은 개발자가 보았을 때 공백 문자열이 NULL로 들어가는 건지 혼동할 수 있기 때문
